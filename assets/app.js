...
function renderNav(){
  $("#nav").innerHTML=navFor().map(([v,label,ic])=>`<button class="nav-item ${state.view===v?"active":""}" data-view="${v}">${icon(ic)}<span>${label}</span></button>`).join("");
  $("#nav").insertAdjacentHTML("afterend",`<div class="nav-extra"><button class="nav-item">${icon("user")}<span>Meu perfil</span></button><button class="nav-item">${icon("settings")}<span>Configuração</span></button></div>`);
}
...
function home(t){
  if(t==="student") return `
    <div class="welcome"><div><span class="kicker">SECRETARIA ONLINE</span><h1>Olá, Estudante.</h1><p>Submeta e acompanhe as suas solicitações.</p></div><button class="btn primary" data-view="new">${icon("plus")}Nova solicitação</button></div>
    <div class="focus-card"><div><span class="process-id">${process.id}</span><h2>${process.subject}</h2><div class="meta"><span>${process.type}</span><span>·</span><span>${process.date}</span></div></div>${pill("Em tramitação","blue")}<div class="focus-bottom"><div><small>Actualmente</small><strong>${process.current}</strong></div><button class="btn secondary" data-open="${process.id}">Acompanhar ${icon("arrow")}</button></div></div>
    <div class="section-line"><h2>Minhas solicitações</h2><button class="link" data-view="requests">Ver todas →</button></div>
    ${requestRows(true)}
  `;
  if(t==="secretary") return `
    <div class="welcome"><div><span class="kicker">SECRETARIA</span><h1>Solicitações recebidas</h1><p>Receba, verifique e encaminhe os processos.</p></div></div>
    <div class="work-card">${workRow("EXP-2026-00125","Exposição dirigida ao Reitor","João Manuel","17/09/2026","Recebida")}</div>
  `;
  if(t==="office") return `
    <div class="welcome"><div><span class="kicker">REITORIA</span><h1>Solicitações recebidas</h1><p>Analise os processos e solicite os pareceres necessários.</p></div></div>
    <div class="work-card">${workRow(process.id,process.subject,process.applicant,process.date,"Recebida")}</div>
  `;
  if(t==="opinion") return `
    <div class="welcome"><div><span class="kicker">ÁREA DE PARECER</span><h1>Processos que requerem intervenção</h1></div></div>
    <div class="work-card">${workRow(process.id,process.subject,"Gabinete do Reitor",process.date,"Aguarda parecer")}</div>
  `;
  return `
    <div class="welcome"><div><span class="kicker">REITORIA</span><h1>Processos aguardando decisão</h1><p>Consulte os pareceres e tome a decisão.</p></div></div>
    <div class="work-card">${workRow(process.id,process.subject,`${process.opinions.filter(x=>x[2]==="done").length} pareceres recebidos`,process.date,"Aguarda decisão")}</div>
  `;
}
